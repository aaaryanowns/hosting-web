...
def get_db():
    db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'storage', 'arfathost.db')
...
def create_app():
    app = Flask(__name__)
    app.config['SECRET_KEY'] = 'arfathost_ultra_pro_max_99'
    app.config['BASE_STORAGE'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'storage', 'instances')
        ...
        if act in ['start', 'restart']:
            ...
            if folder in running_procs or (old_pid and psutil.pid_exists(old_pid)):
                try:
                    ...
                    try:
                        os.killpg(os.getpgid(t_pid), signal.SIGTERM)
                    except Exception:
                        os.kill(t_pid, signal.SIGTERM)
                except Exception:
                    pass

            # Resolve startup safely INSIDE the instance directory. Older database
            # records may contain an absolute path; never pass that path directly
            # to Popen because it breaks when the project is moved (e.g. Pydroid).
            srv = db.execute('SELECT startup FROM servers WHERE folder=?', (folder,)).fetchone()
            startup_file = (srv['startup'] if srv and srv['startup'] else 'main.py').strip()
            base_real = os.path.realpath(path)

            if os.path.isabs(startup_file):
                # Convert an old absolute path belonging to this instance to a
                # relative path. If it points elsewhere, keep only its filename.
                candidate_real = os.path.realpath(startup_file)
                if candidate_real.startswith(base_real + os.sep):
                    startup_file = os.path.relpath(candidate_real, base_real)
                else:
                    startup_file = os.path.basename(startup_file)

            startup_path = os.path.realpath(os.path.join(path, startup_file))
            if not startup_path.startswith(base_real + os.sep) and startup_path != base_real:
                db.close()
                with open(log_file_path, 'a') as f:
                    f.write(f"\n[{now}] ❌ Invalid startup path: {startup_file}\n")
                return jsonify({'status': 'error', 'msg': 'Invalid startup path'})

            # If the configured startup file is missing, automatically choose a
            # script in the instance root. This makes newly uploaded fak.py/main.py
            # files start without manually editing the database.
            if not os.path.isfile(startup_path):
                ...
                for name in sorted(os.listdir(path)) if os.path.isdir(path) else []:
                    ...
                    if os.path.isfile(full) and name.lower().endswith(('.py', '.js')):
                        candidates.append(name)
                if candidates:
                    startup_file = candidates[0]
                    startup_path = os.path.join(path, startup_file)
                    db.execute('UPDATE servers SET startup=? WHERE folder=?', (startup_file, folder))
                    db.commit()
                else:
                    db.close()
                    with open(log_file_path, 'a') as f:
                        f.write(f"\n[{now}] ❌ No startup script found. Upload a .py or .js file.\n")
                    return jsonify({'status': 'error', 'msg': 'No startup script found'})

            f_log = open(log_file_path, 'a')
            f_log.write(f"\n[{now}] ▶️ Starting {startup_file}...\n")
            f_log.flush()
            try:
                if startup_file.lower().endswith('.js'):
                    command = ['node', startup_file]
                else:
                    # Use the same Python interpreter that runs the hosting panel.
                    # This is important on Pydroid/Android where python3 may differ.
                    command = [sys.executable, startup_file]
                try:
                    proc = subprocess.Popen(command, cwd=base_real, stdout=f_log, stderr=f_log, preexec_fn=os.setsid)
                except TypeError:
                    proc = subprocess.Popen(command, cwd=base_real, stdout=f_log, stderr=f_log)
            except Exception as e:
                f_log.write(f"[{now}] ❌ Failed to launch: {e}\n")
                f_log.close()
                db.close()
                return jsonify({'status': 'error', 'msg': str(e)})

            running_procs[folder], start_times[folder] = proc, time.time()
            db.execute('UPDATE servers SET pid=?, status=? WHERE folder=?', (proc.pid, 'Online', folder))
            db.commit()
            db.close()
            f_log.write(f"[{now}] 🚀 Instance STARTED Successfully (PID {proc.pid})\n")
            f_log.flush()
            return jsonify({'status': 'started', 'pid': proc.pid, 'startup': startup_file})
        elif act == 'stop':
            row = db.execute('SELECT pid FROM servers WHERE folder=?', (folder,)).fetchone()
